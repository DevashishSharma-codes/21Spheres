...
const ALL_EVENTS = [
  { day: 15, title: "21Spheres Demo", type: "blue", minStep: 1 },
  ...
  { day: 19, title: "21Spheres Demo", type: "blue", minStep: 4 },
...
export const BookingSection = () => {
  ...
  return (
    <section
      ...
      className="relative z-10 w-full min-h-screen lg:h-screen lg:max-h-screen flex flex-col justify-center bg-paper text-ink py-8 sm:py-12 lg:py-4 pt-16 sm:pt-20 lg:pt-12 border-t border-ink/10 select-none overflow-hidden"
      style={{
        backgroundImage: `radial-gradient(#17130f 1px, transparent 1px)`,
        backgroundSize: "24px 24px",
      }}
    >
      <div className="max-w-[1480px] mx-auto px-3 sm:px-6 md:px-10 w-full flex flex-col justify-center h-full">
        ...
        <div className="flex justify-center mb-3 sm:mb-5">
          <motion.div
            ...
            className="w-fit bg-white border border-black/15 shadow-lg rounded-none px-4 sm:px-10 py-2.5 sm:py-4 flex flex-col items-center text-center max-w-3xl"
          >
            <h2 className="font-outfit text-lg sm:text-3xl lg:text-4xl font-normal tracking-tight text-black leading-tight">
              Reserve Your Architecture Sprint
            </h2>
            <p className="mt-1 font-outfit text-black/75 text-[10px] sm:text-xs font-normal leading-relaxed">
        ...
        <motion.div
          ...
          viewport={{ once: true, amount: 0.1 }}
          transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
          className="w-full border border-black/15 bg-white shadow-2xl overflow-hidden grid grid-cols-1 lg:grid-cols-12 flex-1 min-h-0 lg:min-h-[640px] max-h-none lg:max-h-[calc(100vh-140px)]"
        >
          
          {/* Left Column (5 Cols): Dark Pitch Black Box (Valley Reference) */}
          <div className="lg:col-span-5 bg-[#09090b] text-white p-5 sm:p-8 lg:p-10 flex flex-col justify-between relative overflow-hidden space-y-6 lg:space-y-0">
            
            {/* Top Content */}
            <div>
              <div className="inline-block border border-white/20 bg-white/5 px-2 py-0.5 text-[8px] sm:text-[10px] font-mono font-semibold uppercase tracking-widest text-white/80 mb-4 sm:mb-6">
                YOUR CALENDAR, 30 DAYS FROM NOW
              </div>

              {/* Main Headline */}
              <h3 className="font-outfit text-2xl sm:text-4xl lg:text-4xl xl:text-[42px] font-light tracking-tight text-white leading-[1.08] mb-5 sm:mb-8">
                See what 21Spheres fills your calendar with.
              </h3>

              {/* Interactive Stepped Progress Bar (Valley Style) */}
              <div className="mb-6 sm:mb-8">
                <div className="font-outfit text-xs sm:text-sm text-white/90 font-light mb-3 sm:mb-4 flex items-center gap-1.5">
                  <span>21Spheres ships</span>
                  <strong className="font-semibold text-white text-sm sm:text-base">{currentVal}</strong>
                  <span>commits & outbounds a day</span>
                </div>

                {/* Range Line Slider with Discrete Step Dots */}
                <div className="relative w-full h-2 bg-white/10 rounded-full flex items-center my-4 sm:my-6">
                  ...
                  <motion.div
                    className="absolute w-6 h-6 sm:w-8 sm:h-8 rounded-none bg-white border border-black/20 outline outline-4 outline-white/15 flex items-center justify-center -ml-3 sm:-ml-4 z-20 cursor-grab active:scale-110"
                    animate={{
                      left: `${percent}%`,
                    ...
                    transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
                  >
                    <LogoMark className="w-3.5 h-3.5 sm:w-4.5 sm:h-4.5 text-black shrink-0" />
                  </motion.div>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-none bg-white hover:bg-white/90 text-black px-5 sm:px-6 py-2.5 sm:py-3 font-outfit text-xs sm:text-sm font-semibold tracking-wide transition-all duration-300 shadow-md cursor-pointer mb-4 sm:mb-6"
              >
            ...
            <p className="font-outfit text-[10px] sm:text-xs text-white/60 font-light leading-relaxed border-t border-white/10 pt-3 sm:pt-4">
              Most teams using traditional agencies wait 6+ months for a v1. With 21Spheres, you ship high-scale AI-native software in 30 days.
            </p>

          </div>

          {/* Right Column (7 Cols): Crisp White Working Calendar (Valley Reference) */}
          <div className="lg:col-span-7 bg-white p-3.5 sm:p-6 lg:p-8 flex flex-col justify-between overflow-x-hidden">
            
            <div>
              {/* Working Calendar Header with Live Month Navigation */}
              <div className="flex items-center justify-between pb-2.5 sm:pb-4 mb-2.5 sm:mb-3 border-b border-black/10">
                <div className="flex items-center gap-2">
                  <h4 className="font-outfit text-xl sm:text-3xl font-normal text-black tracking-tight">
                ...
                <div className="flex items-center gap-1.5 text-black">
                  <button
                    ...
                    className="p-1 sm:p-1.5 hover:bg-black/5 border border-black/10 rounded-full transition-colors cursor-pointer active:scale-95 text-black/70"
                  >
              ...
              {/* Days of Week Header */}
              <div className="grid grid-cols-7 text-center font-mono text-[9px] sm:text-xs font-semibold text-black/40 uppercase mb-1.5 sm:mb-2">
                {DAYS_OF_WEEK.map((d) => (
                  <span key={d}>{d}</span>
                ))}
              </div>
              ...
              <div className="grid grid-cols-7 gap-1 sm:gap-1.5 text-xs">
                ...
                {Array.from({ length: startingDayOfWeek }).map((_, i) => {
                  ...
                  return (
                    <div
                      key={`prev-${i}`}
                      className="p-0.5 sm:p-1.5 min-h-[44px] sm:min-h-[64px] lg:min-h-[76px] border border-black/5 bg-black/[0.01] opacity-35 select-none"
                    >
                      <span className="font-mono text-[8px] sm:text-[10px] text-black/40">{prevDayNum}</span>
                    </div>
                  );
                })}
                ...
                {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((day) => {
                  ...
                  return (
                    <div
                      ...
                      className={`p-0.5 sm:p-1.5 min-h-[44px] sm:min-h-[64px] lg:min-h-[76px] border transition-all duration-200 cursor-pointer flex flex-col justify-start overflow-hidden relative ${
                        isSelected
                    ...
                    >
                      <div className="flex items-center justify-between mb-0.5">
                        <span
                          className={`font-mono text-[8px] sm:text-[11px] font-medium block ${
                        ...
                        <AnimatePresence>
                          {events.map((evt, idx) => (
                            <motion.div
                              ...
                              className={`px-0.5 sm:px-1.5 py-0.5 rounded-xs text-[6.5px] sm:text-[8px] lg:text-[9px] font-semibold truncate leading-tight ${
                                evt.type === "orange"
              ...
              <div className="mt-2.5 sm:mt-3 pt-2.5 sm:pt-3 border-t border-black/10">
                <span className="block font-outfit text-[10px] sm:text-[11px] font-semibold text-black/60 uppercase tracking-wider mb-1.5">
                ...
                <div className="flex flex-wrap gap-1 sm:gap-1.5">
                  {TIME_SLOTS.map((slot) => (
                    <button
                      ...
                      className={`px-2 sm:px-2.5 py-1 font-outfit text-[10px] sm:text-[11px] rounded-md border transition-all cursor-pointer ${
            ...
            <div className="mt-2.5 sm:mt-3 pt-2.5 sm:pt-3 border-t border-black/10 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2">
              <div className="flex items-center gap-1.5 text-xs font-outfit">
                <Clock size={14} className="text-black/60 shrink-0" />
                <span className="font-light text-black/70 text-[10px] sm:text-xs">
                  Selected: <strong className="font-semibold text-black">{MONTH_NAMES[month]} {selectedDay}, {year} at {selectedTimeSlot}</strong>
                </span>
              </div>
              <button
                onClick={handleBookingClick}
                className="w-full sm:w-auto bg-black hover:bg-black/85 text-white font-outfit text-xs font-semibold px-4 py-2 rounded-full transition-all duration-200 cursor-pointer shadow-md flex items-center justify-center gap-1.5"
              >
